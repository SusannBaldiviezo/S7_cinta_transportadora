                        ...
                        int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        ...
        /* ── VEREDICTO DE FRUTA ──
         * aceptada        → 45°  (derecha)   servo_disparar_maduro
         * pintón / verde  → 90°  (centro)    no mueve si ya está ahí
         * podrida         → 135° (izquierda)
         * Solo notificamos la tarea de la válvula — NO bloqueamos el hilo MQTT */
        else if (strcmp(topic, TOPIC_FRUTA) == 0 && sistema_activo) {
            if (strcmp(data, "aceptada") == 0 || strcmp(data, "maduro") == 0) {
                servo_disparar_maduro();   /* → 45° derecha */
            } else if (strcmp(data, "pintón") == 0 || strcmp(data, "pinton") == 0 ||
                       strcmp(data, "verde")  == 0) {
                servo_disparar_pinton();   /* → 90° centro  */
            } else if (strcmp(data, "podrida") == 0 || strcmp(data, "podrido") == 0) {
                servo_disparar_podrido();  /* → 135° izquierda */
            }
        }
        break;
